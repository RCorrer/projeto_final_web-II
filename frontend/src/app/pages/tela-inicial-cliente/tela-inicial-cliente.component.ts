...
export class TelaInicialClienteComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.authService.isCliente() && clienteId) {
      console.log(
        `tela-inicial-cliente: usuario é cliente. Buscando solicitações para Clientes.id: ${clienteId}`
      );
      this.carregarMinhasSolicitacoes(clienteId);
    } else {
      console.warn(
        "tela-inicial-cliente: usuário não é cliente ou idRole (Clientes.id) não encontrado."
      );
    ...
    this.solicitacaoService.solicitacoesCliente$.subscribe(
      (solicitacoesRecebidas) => {
        ...
        console.log(
          "tela-inicial-cliente: lista de solicitações (do backend) atualizada no componente:",
          this.solicitacoes
        );
      }
    );
  ...
  carregarMinhasSolicitacoes(clienteId: string) {
    this.isLoading = true;
    this.solicitacoes = [];
    this.solicitacaoService.fetchSolicitacoesPorClienteId(clienteId).subscribe({
      next: () => {
        console.log(
          "tela-inicial-cliente: chamada fetchSolicitacoesPorClienteId completada."
        );
      },
      error: (err: any) => {
        console.error(
          "tela-inicial-cliente: erro ao chamar fetchSolicitacoesPorClienteId:",
          err
        );
        this.solicitacoes = [];
        this.isLoading = false;
      ...
      complete: () => {
        this.isLoading = false;
        console.log(
          "tela-inicial-cliente: carregamento de solicitações finalizado."
        );
      },
    });
  }
  ...
  alterarEstadoSolicitacao(evento: { id: string; novoEstado: string }) {
    console.log(
      `Mock: Alterando estado da OS ${evento.id} para ${evento.novoEstado}`
    );
